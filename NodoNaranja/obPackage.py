...
class obPackage:
	...
	'''
		...
		MOD: ---
	'''                                           
	def __init__(self, packetCategory = -1, nodeID = -1, neighborID = -1, blueAddressIP = '999.999.999.999', blueAddressPort = 0000, fileIDByte1 = 0, fileIDRest = 0, chunkID = 0, chunkPayload = b'0',filename="",chunkTotal = 0, completeResponse = False):
	...
	def serialize(self,tipo):
		...
		if (tipo == 0 or tipo == 7): #chunk
			...
			bytePacket = struct.pack('!bBH',self.packetCategory,self.fileIDByte1,self.fileIDRest)
			bytePacket += self.fileName.encode(encoding='UTF-8',errors='replace')
		elif tipo == 22: #compelte request/response
			bytePacket = struct.pack('!bBHI?',self.packetCategory,self.fileIDByte1,self.fileIDRest, self.chunkTotal, self.completeResponse)
			bytePacket += self.fileName.encode(encoding='UTF-8',errors='replace')
	...
	def unserialize(self, bytePacket,tipo):
		...
		elif tipo == 22: #complete request/response
			processedPacket = struct.unpack('!bBHI?', bytePacket[:struct.calcsize('!bBHI?')])
			self.packetCategory = processedPacket[0]
			self.fileIDByte1 = processedPacket[1]
			self.fileIDRest = processedPacket[2]
			self.chunkTotal = processedPacket[3]
			self.fileName = bytePacket[struct.calcsize('!bBHI?'):].decode("utf-8")
